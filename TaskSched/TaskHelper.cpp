...
HRESULT TaskSchedSvc::Init(PCWSTR machineName, PCWSTR username, PCWSTR domain, PCWSTR password) {
	auto hr = _svc.CoCreateInstance(__uuidof(TaskScheduler));
	if (FAILED(hr))
		return hr;

	hr = _svc->Connect(CComVariant(machineName), CComVariant(username), CComVariant(domain), CComVariant(password));
	if (FAILED(hr))
		return hr;
...
CString TaskHelper::FormatMessage(DWORD error) {
	LPTSTR buffer;
	if (::FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM, nullptr, error, 0, (LPTSTR)&buffer, 0, nullptr)) {
		CString msg(buffer);
		::LocalFree(buffer);
		return msg;
	}
